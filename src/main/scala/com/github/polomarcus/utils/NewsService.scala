...
object NewsService {
  ...
  def enrichNewsWithClimateMetadata(newsDataset: Dataset[News]): Dataset[News] = {
    newsDataset.map { news =>
      val containsClimateWords = ClimateService.isClimateRelated(news.title + " " + news.description)
      val enrichedNews = News(
        ...
        news.url,
        news.urlTvNews,
        containsClimateWords, // Set the value here
        news.media
      )
      enrichedNews
    }
  ...
  def getNumberOfNews(dataset: Dataset[News]): Long = {
    dataset.filter(news => com.github.polomarcus.utils.ClimateService.isClimateRelated(news.description)).count() //@TODO look a the Spark API to know how to count
    return 3 // code here
  }
}
